Prerequisites:
    EKS should have Permissions to Use S3 Bucket or  Instance should have attached Permission to IAM role to Access S3 Bucket
    Here We Are Using Loki-Distributed Since It is the only Type That Woks With S3 bucket Properly.
    Loki With logs stored on S3 Bucket.
...
helm upgrade --install [RELEASE_NAME] --namespace=<NAMESPACE> grafana/loki-distributed --values values.yaml
